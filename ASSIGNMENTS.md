...
- `bsnd_basic`(K=16)/`bsnd_multi_batch`(K=32) 在 `ENABLED_PARAMS` 里测过 BSND 一层寻址,但 **`K=2048` 的用例是 PA_BSND**——生产形态(BSND+K=2048)未被本地 CI 覆盖,须 P1 自测;全部 `sparse_block_size=1`——散读由 SFA 自己扛（内部做"指令缩减 + 搬运聚合"），不是 B 要解决的问题；
