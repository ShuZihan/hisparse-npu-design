...
> **读前须知（接 01/02 的比方）**
>
> 前两篇把 HiSparse 讲成"小书桌 + 大仓库 + 按需搬页"。本篇讨论：这套搬运机制要同时服务**两类不同的书**(NSA 模型 和 DeepSeek-V4)，哪些能共用、哪些得分开。
>
> 一句话直觉：**搬运系统(物流)能共用，书本身(KV 格式)不能共用。**
>
> - **物流(可共用)**：进出库、书桌摆放、LRU 淘汰、按需搬页调度——不管搬哪类书，流程一样。本文叫它 `HiSparseResidencyRuntime`。
> - **书本身(必须分开)**：
>   - **NSA 书(GLM-5 / V3.2)**：普通书，一页就是一页(`compress_ratio=1`)，要看哪页直接报原始页码，整本 attention 就读这些选中的页。
>   - **DSV4 书**："每 4 页压成 1 页摘要"的书(`compress_ratio=4`)，attention 是**复合**的——一部分永远读最近几页(SWA 滑动窗口，不下沉仓库)，另一部分读压缩摘要(C4)。**HiSparse 只负责搬运摘要那一部分**。
>
> 全文结论(§3.1)即此句展开：**runtime 共用、pool 分开**。§3.8 证明"书为什么不能共用"(4 个契约都不同)，§3.9 进一步证明"连 attention 里的角色都不同"(6 个维度)。
...
> **怎么读这张表**：每行都是"同一件事，两类书各怎么做"。最关键两行是 `compress_ratio`(1 vs 4)和 attention 接口(NSA 把选中页当**唯一**KV 来源，DSV4 只当 SWA 之外的**补充**来源)——其余差异基本由这两点派生。
...
| Attention backend | `nsa_backend.py` → `flashmla_kv` / `flashmla_sparse` / FA3 / tilelang | `deepseek_v4_backend.py` → FlashMLA with `extra_indices_in_kvcache` |
...
SGLang NSA HiSparse (GPU 参考实现)
  ...
  → attention 对接 npu_sparse_flash_attention（路线 A=PA_BSND remap 当前主线，路线 B=BSND hot-slot 优化/备选，路线 C=改算子兜底）
```
...
> **这一节在证明"书不能共用"。** 哪怕两类书都说"第 100 页"，四件事全不一样：① 第 100 页指哪一页(token 语义) ② 一页的字怎么排(内存 layout) ③ 怎么写进页里(写入接口) ④ attention 怎么读走(消费方式)。所以物流可共用、KV pool 必须各做一套。
...
4. **attention kernel 收到错误格式的 KV**：NSA backend 期望的主 `kv_cache` shape 和 DSV4 backend 期望的 `extra_k_cache` shape 完全不同

### 3.9 GLM Sparse Attention 与 DSV4 C4 的结构性差异（超越压缩比）

> **这一节回答一个更深的问题**："不就是 DSV4 多了个压缩吗，把压缩比设成参数不就统一了？" 不行——**两类书的 sparse attention 在模型里扮演的角色根本不同**。最核心一点(§3.9.2)：NSA 的 sparse attention 是**这一层的全部**(选中页即唯一 KV 来源)；DSV4 的 C4 只是**复合 attention 的一个分支**(主力是 SWA 看最近几页，C4 只补远处压缩信息)。下面 6 个维度的差异都由此长出。
...
GLM 走 SGLang 的 NSA 路线。`GlmMoeDsaForCausalLM` 直接继承 `DeepseekV2ForCausalLM`（`glm4_moe.py:1483`），使用 `nsa_backend.py` 的 `flashmla_kv` / `flashmla_sparse` / FA3 / tilelang 等 backend。
...
- **GLM/NSA**：`page_table_1`（即 `top_k_device_locs`）直接作为 sparse attention 的**唯一主 KV indices**，FP8 KV 时进入 `flash_mla_with_kvcache(indices=...)`，BF16 KV 时进入 `flash_mla_sparse_fwd(indices=...)`，都指向单一的 `kv_cache`。
