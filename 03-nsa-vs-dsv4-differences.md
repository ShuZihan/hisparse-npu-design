...
> **读前须知（接 01/02 的比方，再加一层）**
>
> 前两篇把 HiSparse 讲成"**小书桌 + 大仓库 + 按需搬页**"。本篇讨论的是：这套搬运机制要同时服务**两类不同的书**(NSA 模型 和 DeepSeek-V4)，哪些能共用、哪些得分开。
>
> 关键直觉一句话：**搬运系统(物流)能共用，书本身(KV 格式)不能共用。**
>
> - **物流(可共用)**：仓库进出库、书桌摆放、LRU 淘汰、按需搬页的调度——不管搬的是哪类书，流程一样。本文叫它 `HiSparseResidencyRuntime`。
> - **书本身(必须分开)**：两类书的"开本和装订"完全不同——
>   - **NSA 书(GLM-5 / V3.2)**：普通书，一页就是一页(`compress_ratio=1`)，要看哪页直接报原始页码。整本书的 attention 就读这些选中的页。
>   - **DSV4 书**：一本"每 4 页压成 1 页摘要"的书(`compress_ratio=4`)，而且它的 attention 是**复合**的——一部分永远读最近几页(SWA 滑动窗口，不下沉仓库)，另一部分读压缩摘要(C4)。**HiSparse 只负责搬运摘要那一部分**，SWA 不管。
>
> 所以全文的结论(§3.1)就是这句话的展开：**runtime 共用、pool 分开**。§3.8 证明"书为什么不能共用"(4 个契约都不同)，§3.9 进一步证明"连 attention 里的角色都不同"(6 个维度)。读密集的对比表时，回到这个"两本书 + 共用物流"的画面即可。
...
> **怎么读这张表**：每一行都是"同一件事，两类书各怎么做"。最该记住的两行是 `compress_ratio`(1 vs 4，即"一页一页" vs "4 页压成 1 页")和最后两行 attention 接口(NSA 把选中的页当**唯一**KV 来源，DSV4 只把它当 SWA 之外的**补充**来源)——其余差异基本都是这两点派生出来的。
...
> **这一节在证明"书不能共用"。** 用一句话概括下面四个小节：哪怕两类书都说"第 100 页"，这句话的含义、这页的字怎么排版、怎么写进去、attention 怎么读出来——四件事全不一样。所以搬运物流可以共用，但"书本身的格式"(KV pool)必须各做一套。四个契约依次是：① 第 100 页指的是哪一页(token 语义) ② 一页的字怎么排(内存 layout) ③ 怎么把字写进页里(写入接口) ④ attention 怎么把页读走(消费方式)。
...
> **这一节在回答一个更深的问题**：有人会想"不就是 DSV4 多了个压缩吗，把压缩比设成参数不就统一了？" 本节证明不行——**两类书的 sparse attention 在整个模型里扮演的角色根本不是一回事**。最核心的一点(§3.9.2)：NSA 的 sparse attention 是**这一层的全部**(选中的页就是 attention 的唯一 KV 来源)；DSV4 的 C4 只是**复合 attention 里的一个分支**(主力是 SWA 滑动窗口看最近几页，C4 只是额外补一点远处的压缩信息)。角色不同，下面 6 个维度(候选空间、层分布、接入点、参数语义……)的差异都是从这里长出来的。
