...
> 2. **页码怎么对**(§2.3.2)：GPU attention 直接吃"绝对物理页码"；当前 GLM-5 的 NPU SFA 生产调用是 `layout_query="TND"` + `layout_kv="PA_BSND"` + `block_table`，吃的是"逻辑页码"，算子内查 block_table 翻译。所以最小改造主线应先保留 PA_BSND，重映射 `block_table` 到 hot buffer。SFA 内核源码也有 BSND 一层寻址(`block_table=None`、`sparse_indices` 经 `+s2StartIdx` 后当 S 轴下标)，它更像 GPU，但需要改 query/rope/output 调用形态，作为优化/备选路线。
...
架构对标 GPU HiSparse：AscendC kernel 在 kernel 内完成 hit/miss/LRU/copy，输出 hot slot 给 SFA。但"kernel 内跨总线直读 host"在 NPU 上是**高风险实验路线（路径①）**——能力官方支持，但 P0b 须验证 kernel 内 `DataCopy` 对 registered host 地址的可用性 + EP 形态带宽 + Graph capture（§2.2.2 的两条注册路措辞相反）。**基准退路是路径②（staged DMA，必须先跑通）**：kernel 只判定、搬运退到 kernel 外。**寻址当前主线 A（PA_BSND block_table 重映射，不改算子）**——保持 GLM-5 当前 `layout_query="TND"`、`layout_kv="PA_BSND"`、`block_table` 调用形态，让 `block_table` 指向 hot buffer block。BSND 一级直址分支仍然有价值，但它需要切 query/rope/output ABI，作为路线 B 做 golden；改算子的 `PHYSICAL_HOT_SLOT` 路线 C 仅作兜底。
...
每步 decode (per layer):
  ...
  → SFA adapter:
      输入: logical top-k, top_k_device_locs, hot block/page 映射
      输出: remapped sparse_indices / block_table
  → npu_sparse_flash_attention(           # 路线 A：保持 PA_BSND，不改算子
...
> **难点②：页码怎么对。** attention 按清单取 KV，问题是清单号码指向哪个池子。GPU FlashMLA 清单写**绝对物理槽号**，kernel 只做 `idx/page_size` 和 `idx%page_size`；当前 NPU GLM-5 SFA 清单写的是 **S2 逻辑位置**，PA_BSND 分支再查 `block_table` 才得到真实 KV block。

当前结论：**先走 PA_BSND `block_table` 重映射，再验证 BSND hot-slot。** 这是因为 GLM-5 生产调用已经是 `layout_query="TND"` + `layout_kv="PA_BSND"`，保留这套 ABI 改动最小。BSND 一层直址确实存在，但它要求 `layout_query==layout_kv=="BSND"`，query/rope/output 形态都要跟着切。
...
因此路线 A/B 改的不是“把一个字段改名”。它要把 GPU 已经算好的 physical index 契约，落到 SFA 的 `key/value/key_rope + sparse_indices + block_table + actual_seq_lengths` 这组 ABI 上。
...
- BSND 在 CI 有用例但只覆盖小 K：`bsnd_basic` 是 K=16，`bsnd_multi_batch` 是 K=32；K=2048 的 `pa_bsnd` 用例走 PA_BSND。所以 BSND+GLM-5 生产 tuple 不能假定已覆盖。
- `return_softmax_lse=True` 时不支持图模式；HiSparse 当前用 False，不受限。BSND 还要求 `layout_query==layout_kv`。

**三条寻址路线**：

| 路线 | SFA 看到的 KV | 关键做法 | 代价/验证点 | 结论 |
|---|---|---|---|---|
...
| B：BSND hot-slot 直读 | `key/value=[B,device_buffer_size,1,D]`，`block_table=None`，`layout_query="BSND"`，`layout_kv="BSND"` | `sparse_indices` 填 hot slot，例如 `407`；SFA BSND 分支直接读 `boIdx*s2Size+s2Idx` | query/rope/output 要从 TND+PA_BSND 切到 BSND；要处理 `s2StartIdx`；`actual_seq_lengths_kv` 要改成 hot buffer S2；K=2048 生产 tuple 需自测 | 优化/备选路线 |
| C：改 SFA | 新 hot-slot ABI | `sparse_indices` 直接表示 hot slot，或支持 FlashMLA-style encoded KV index | 要改 op host、tiling、kernel、测试和分发 | A/B 都对不齐 baseline 时兜底 |
...
**为什么 SFA 默认用 PA_BSND 内部翻译**（设计选择，非缺陷）：减少 kernel launch、DMA 紧耦合、按 block 连续搬对 SIMD 友好。HiSparse 可以利用这个现有分支先做 block_table remap；BSND 分支则用于验证 token-wise hot-slot 是否值得切 ABI。
...
三处共用同一个 `torch_npu.npu_sparse_flash_attention` 算子，当前参数一致（`sparse_block_size=1`, `layout_kv="PA_BSND"`, `sparse_mode=3`, `attention_mode=2`, 都带 `block_table`）。路线 A/B 只改 Python 侧的调用参数——路线 A 保持 `PA_BSND` 但重映射 `block_table`；路线 B 把 `layout_kv` 切到 `"BSND"`、`block_table=None`、喂小 hot buffer 当 `key/value`、`sparse_indices` 填 hot slot。两者**三处调用都自然受益、且不碰算子源码**；仅路线 C 需改算子内部寻址分支。
...
| GPU 组件 | NPU 对应（新增） | 职责 |
|----------|-----------------|------|
| `HiSparseNSATokenToKVPool` | `NPUHiSparseMLATokenToKVPool` | Device hot buffer，**布局由寻址路线定**：路线 A 用 PA_BSND + 重映射 block_table，路线 B 用 BSND（`sparse_indices` 直填 hot slot，§2.3.2）；只分配 `device_buffer_size` slots |
...
forward_decode() (per layer):
  ...
  │
  ├── SFA adapter:
  │     输入: top_k_tokens, top_k_device_locs, hot page mapping
  │     输出: remapped sparse_indices / block_table
  │
  └── npu_sparse_flash_attention(           # 路线 A：保持 PA_BSND，不改算子
...
| 小池子 PA_BSND remap / BSND hot-slot 是否被 SFA 接受 | 路线 A/B 成立的前提（§2.3.2）。A 保持当前 GLM-5 `TND+PA_BSND+block_table` ABI，但要证明 remapped `block_table` 能让 SFA 读 hot buffer，且 page 放大后的 hot buffer/H2D 成本仍划算。B 的 BSND 一级直址分支确实存在：`realkeyOffset = boIdx*s2Size + s2Idx`（`service_vector_mla.h:233`），但本地 CI 的 BSND 用例仅 `K=16/32`，`K=2048` 走 PA_BSND；生产形态还要验证 query/rope reshape、`s2StartIdx`、`actual_seq_lengths_kv` | **P1 上机 golden 先测 A，再测 B**；生产 tuple：B 多 batch、decode S1=1、K=2048、BF16/FP16、query_rope/key_rope、actual lengths、s2StartIdx |
...
| **P1: SFA 寻址探针** | 先验证路线 A：保持 `layout_query="TND"`、`layout_kv="PA_BSND"`，用 remapped `block_table` 读 hot buffer，并二选一确定 A1 大表 / A2 compact。再验证路线 B：BSND hot-slot 直读，落定 query/rope 转 BSND、`s2StartIdx` 扣减、`actual_seq_lengths_kv` 改义。本地 CI 仅覆盖 BSND K=16/32，生产形态须自测 | 单层 golden test：路线 A/B 与 full-resident 对齐 |
| **P2: AscendC residency kernel** | hit/miss/LRU/copy 全流程（路径②则为 judge-only kernel + host 侧 staged DMA） | 对齐 Python 参考实现 |
| **P3: 寻址路线定稿** | P1 通过则用路线 A 或 B（不改算子）做多层 decode 对齐；P1 不通过才退路线 C（改 SFA 内部寻址 ABI） | 多层 decode 对齐 full-resident |
