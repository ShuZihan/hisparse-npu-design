...
架构对标 GPU HiSparse：AscendC kernel 在 kernel 内完成 hit/miss/LRU/copy，输出 hot slot 给 SFA。但"kernel 内跨总线直读 host"在 NPU 上是**高风险实验路线（路径①）**——能力官方支持，但 P0b 须验证 kernel 内 `DataCopy` 对 registered host 地址的可用性 + EP 形态带宽 + Graph capture（§2.2.2 的两条注册路措辞相反）。**基准退路是路径②（staged DMA，必须先跑通）**：kernel 只判定、搬运退到 kernel 外。**寻址首选路线 A（BSND、不改算子）**——hot buffer 以 BSND 布局当 `key/value`、`block_table=None`、`sparse_indices` 填 hot slot。SFA 内核确有 BSND 一级直址分支（`isPa=false`），但本地 CI 仅在 `K=16/32` 测过 BSND（`K=2048` 走 PA_BSND），故生产形态须经 P1 golden 验证（依据与 ABI 见 §2.3.2；改算子的 `PHYSICAL_HOT_SLOT` 路线 C 仅作兜底）。
...
每步 decode (per layer):
  ...
  → npu_sparse_flash_attention(           # 路线 A：不改算子（完整 ABI 见 §2.3.2，下为骨架）
      query=q_nope, query_rope=q_pe,       # ← query 须从 TND 切到 BSND（layout 必须与 kv 一致）
      key=hot_buffer, value=hot_buffer,    # BSND 布局，整块 hot buffer
      key_rope=hot_buffer_rope,
      sparse_indices=top_k_device_locs,    # 减去 s2StartIdx 基址后填（须确认 decode 下=0）
      actual_seq_lengths_kv=<hot buffer 有效 S2，非原始 seq_len>,
      block_table=None, layout_query="BSND", layout_kv="BSND", sparse_block_size=1)
```
...
路径 ② 的代价：**每步 decode 多一趟 device→host→device 往返**（kernel 出来拿 miss list、发 DMA、等完成、再进 SFA），打断 CANN Graph 单图捕获。但正确性可保证。**搬运 API 须按 hot buffer layout 选**：仓库既有的 `transfer_kv_dim_exchange` 强制 `device_k.dim()==5`、按 **page** 搬全 layer、且 SGLang 仅在 `layout=="page_first_direct"` 时调它（`transfer_kv_dim_exchange.cpp:36`、`memory_pool_host.py:495`）——**只适配路线 A′ 的 PA/page hot buffer**。路线 A 的 **BSND/token hot buffer 与它不匹配，需要新写一个 staged copy（逐 token `aclrtMemcpyAsync` 或新算子）**。两条路差别在 Graph 友好度与一趟往返延迟，**都有退路但路径②的 BSND 搬运 API 需新建**（详见 §2.7.5）。
...
**依据(从"官方文档"升级为"算子内核源码可读"——SFA 真源码在 [`ops-transformer/attention/sparse_flash_attention`](../ops-transformer/attention/sparse_flash_attention);但生产 tuple 仍待 P1 实测)**：
...
- **BSND(`block_table=None`) 一级直址在 CI 有用例,但仅小 K**：[`sparse_flash_attention_paramset.py`](../ops-transformer/attention/sparse_flash_attention/tests/pytest/sparse_flash_attention_paramset.py) 的 `ENABLED_PARAMS` 里 `bsnd_basic`(K=16)/`bsnd_multi_batch`(K=32) 是 BSND,但 **`K=2048` 的 `pa_bsnd` 用例走的是 PA_BSND**。即"BSND 一层寻址"被测过、"BSND + K=2048 生产形态"**未**在本地 CI 测过(全部 `sparse_block_size=1`)。公开 `torch_npu` 文档另有 BSND+`sparse_block_count=2048`+图模式示例,故路线 A 有接口依据,但**生产 tuple 须 P1 golden 自测**。
...
**三条寻址路线（按改动深度排序；此前文档基于"NPU 散读是死结、必须自己改寻址"把路线 C 列为推荐，内核源码推翻了该前提——arch35 token-wise 是唯一支持、散读已被算子优化、BSND 一层寻址在内核里存在，故推荐降级为路线 A，待 P1 验证生产 tuple）**：

- **路线 A（推荐·不改算子）**：hot buffer 以 **BSND** 布局当 `key/value` 传入（`block_table=None`），`sparse_indices` 填 hot buffer 内槽号。SFA 内核有 BSND 一级直址分支(`isPa=false`)。这是对位 GPU"扁平池 + 物理槽清单"的最干净路径，但**不是改个 `layout_kv` 字符串就行**——P1 须落定的完整 ABI：
  1. **query 同步切 BSND**：BSND 要求 `layout_query==layout_kv`（`check_valid_param.py:51`），现网是 `layout_query="TND"`+`layout_kv="PA_BSND"`。query 及 `query_rope` 都要从 TND reshape 到 BSND，`key_rope` 同理。
  2. **`s2StartIdx` 基址扣减**：kernel 内 `s2Idx = sparse_indices + s2StartIdx`（`service_vector_mla.h:207`），槽号非绝对填入——须确认 decode 场景 `s2StartIdx=0`，否则喂入前扣除。
  3. **`actual_seq_lengths_kv` 改义**：现为原始 seq_len（`ascend_backend.py:926`）；BSND 下 S2 来自 key shape / actual len，须改成 hot buffer 的有效 S2（= `device_buffer_size`）。
  4. **生产 tuple golden**：本地 CI 的 BSND 仅测到 `K=16/32`，`K=2048` 是 PA_BSND——故 `B 多 batch / S1=1 / K=2048 / BF16+FP16 / rope / CP` 都要自测，不能假定 CI 已覆盖。
- **路线 A′（不改算子·页粒度）**：保持 **PA_BSND**,`key/value` 传 hot buffer 池,`sparse_block_size` 仍为 1(不受 arch35 token-wise 限制)。关键细节：PA 分支是 `blkTableIdx = s2Idx/blockSize` 后查 `block_table`(`service_vector_mla.h:225`)，而 PA 的逻辑 S2 = `maxBlockNumPerBatch * blockSize`(`tiling.cpp:1899`)。所以"重映射 block_table"有**两个互斥变体,P1 须二选一**：
  - **A′-1（保留大表）**：`sparse_indices` 仍填**原始长序列逻辑 token**,则 `block_table` 必须仍覆盖原始逻辑块号空间(大小 = 原始块数),只是表项指向 hot buffer 物理块——表不能 compact。
  - **A′-2（compact 重写）**：把 `sparse_indices` 重写到 **compact hot 逻辑空间**(0..hot_blocks),`block_table` 才能只列 hot blocks。代价是每步要重算 sparse_indices 的逻辑→compact 映射。
  - 代价：页粒度（block_size 整数倍驻留）。query 可继续用 TND（PA_BSND 允许 query/kv 布局不一致），改动比 A 更局部。
...
forward_decode() (per layer):
  ...
  │
  └── npu_sparse_flash_attention(           # 路线 A：不改算子（完整 ABI 见 §2.3.2）
        query=q_nope, query_rope=q_pe,       # query 同步切 BSND
        key=hot_buffer, value=hot_buffer,    # BSND 布局
        key_rope=hot_buffer_rope,
        sparse_indices=top_k_device_locs,    # 减 s2StartIdx 后填
        actual_seq_lengths_kv=<hot buffer S2>,
        block_table=None, layout_query="BSND", layout_kv="BSND", sparse_block_size=1)
```
...
| 小池子 BSND/重映射 block_table 是否被 SFA 接受 | 路线 A/A′ 成立的前提（§2.3.2）。**SFA 内核确有 BSND 一级直址分支**：`realkeyOffset = boIdx*s2Size + s2Idx`（`service_vector_mla.h:233`）。但**本地 CI 的 BSND 用例仅 `K=16/32`**,`K=2048` 走 PA_BSND——生产形态未被 CI 覆盖。**剩余未验证点有四**：① key 换成 device_buffer_size 小 hot buffer（公式里 `s2Size` 变小）;② BSND 要求 `layout_query==layout_kv`,须把 query 从现网 TND 切到 BSND（连带 query_rope/key_rope reshape）;③ 索引含 `s2StartIdx` 窗口基址,槽号非绝对填入;④ `actual_seq_lengths_kv` 现为原始 seq len（`ascend_backend.py:926`),BSND 下须改为 hot buffer 的 S2 | **P1 上机 golden**（用生产 tuple：B 多 batch、decode S1=1、K=2048、BF16/FP16、query_rope/key_rope、actual lengths、s2StartIdx；风险低于此前但非"已实测"） |
...
| **P1: SFA 寻址探针** | 验证不改算子的路线 A/A′ 是否成立。路线 A 须落定完整 ABI（query/rope 转 BSND、`s2StartIdx` 扣减、`actual_seq_lengths_kv` 改义，§2.3.2）并用**生产 tuple**(B 多 batch / S1=1 / K=2048 / BF16+FP16 / rope / actual lengths)做 golden;路线 A′ 须二选一确定 block_table 变体（A′-1 大表 / A′-2 compact）。本地 CI 仅覆盖 BSND K=16/32,生产形态须自测 | 单层 golden test：路线 A/A′ 与 full-resident 对齐 |
...
**架构判断（结合 §2.2.2 与 §2.7.3.1）**：判定平面四步全走 device 上 proven 的 SIMD API、不依赖 host 直读，因此它本身就是一个**完整的路径②判定实现**——判定全程在 device kernel 内、只把紧凑 miss_list 交给 host 发 staged DMA。**这条判定平面不必等 P0b 就能开写。** P0b 因此从"成败门控"降为"优化"：通过则把搬运也吸进 kernel（路径①，Graph 更友好）；不通过则判定平面 + host staged DMA 照跑（路径②，多一趟往返、Graph 需分段）。**注意路径②的搬运侧并非全现成**：BSND/token hot buffer 的 staged copy API 需新建（既有 `transfer_kv_dim_exchange` 仅适配 PA/page，§2.2）。
