...
> **一句话**：FlashMLA 本就是"给我一张行号清单，我去 KV tensor 里读这几行"的算子。HiSparse 的 swap-in kernel 输出的恰好就是这样一张行号清单，直接对接、无需改动。

关键是两件事:FlashMLA 要什么、HiSparse 给什么。

**① FlashMLA 要什么**：它为 sparse attention 设计，不读全量 KV，而接受 `indices` / `indices_in_kvcache` 参数——"按这些 encoded KV index 去 KV cache tensor 里读"。注意，`physical_row` 和 `page_block_index` 都不是源码里的正式变量名；FlashMLA 测试里实际构造的是 `cur_blocked_indices = block_table_cpu[i, cur_abs_indices // block_size] * block_size + (cur_abs_indices % block_size)`。kernel 内把读到的 `token_index` / `my_indices[i]` 做除法和取模，得到源码变量 `block_index` / `block_idx` 与 `rel_idx_in_block` / `idx_in_block`(`splitkv_mla.cuh:538`)。sparse 路径 **从不解引用 block_table**(sparse 路径的 `block_table` 是传了也被忽略的 vestigial 参数,`flash_mla_interface.py:77` 注明 "Can be None when sparse attention is used")。不开 HiSparse 时，这个 index 是：indexer 给 logical 位置 → page table 在 **Python 侧**翻译成 `indices_in_kvcache` → 喂给 FlashMLA。

**② HiSparse 给什么**：决定性细节——**hot buffer 不是另开的 tensor，它就是 KV cache tensor 里的一段连续区域**。所以 swap-in 写入的 `device_buffer[slot]` 本身就在 KV cache tensor 的同一地址空间里，输出的 `top_k_device_locs` 可以直接作为 FlashMLA sparse 的 encoded KV index。FlashMLA 测试里 `indices_in_kvcache` 也正是这么构造：
...
**③ 对接**：只差形状对齐。FlashMLA `indices` 是 `[batch, seq_len_q, topk]`(decode 时 `seq_len_q=1`)，HiSparse 的 `top_k_device_locs` 是 `[batch, topk]`，`unsqueeze(1)` 即可。

**所以"不需要改 FlashMLA"的本质**：HiSparse 没改 FlashMLA 的输入契约，只是把 `indices_in_kvcache` 的生产者从 page table 换成 swap-in kernel——且因 hot buffer 复用 KV tensor 的地址空间，encoded index 语义一致。这就是 GPU"只写一个 swap-in kernel 就搞定整个 HiSparse"、不动任何 attention 算子的根本原因。

**代码铁证**（`nsa_backend.py:1612-1626`）：FlashMLA sparse 需要的 encoded index 清单 `page_table_1` 由一个**三选一**的分支产出，三条路算出的是**同一个变量**，随后**不加区分地**喂给所有 attention backend。
...
1. **算子不知道也不关心 encoded index 是谁生产的**：三分支产出物变量名相同、下游调用相同，FlashMLA 只是收到一张 `page_table_1` 就照着读。
2. **HiSparse 做的就是"换生产者"**：它用 `swap_in_selected_pages`（一级，直接出 FlashMLA-style encoded index）替换掉默认的 `transform_index_page_table_decode`（两级，logical → 查 block_table → encoded index），算子本身一行不改。
3. **page_size 与寻址级数无关**：默认分支显式传 `page_size=1` 仍走两级翻译——印证了"一格放一个 token"和"要不要查表翻译"是两件独立的事。

> **对 NPU 适配的提示**：GPU 的"零修改"红利建立在 ① hot buffer 与 KV tensor 共用地址空间、② 算子接受 `indices_in_kvcache` 这类 encoded index 清单 这两个前提上。当前 GLM-5 在 NPU 的生产调用是 `layout_query="TND"` + `layout_kv="PA_BSND"` + `block_table`，SFA 收到的是 logical S2 索引，不是 FlashMLA 那种绝对物理槽号。因此 NPU 的最小改造主线不是先切 BSND，而是保持 PA_BSND，通过重映射 `block_table` 让现有 SFA 读 hot buffer。SFA 内核源码确有 **BSND 一层寻址分支**（`isPa=false`：`block_table=None`、`sparse_indices` 经 `+s2StartIdx` 后当 S 轴下标，`service_vector_mla.h:233`），它更像 GPU，但需要把 query/rope/output 一起从当前 TND+PA_BSND 调用形态切过去；本地 CI 的 BSND 用例只覆盖 `K=16/32`（`K=2048` 走的是 PA_BSND），所以 BSND 是优化/备选路线，仍须 golden 验证。改算子的路线 C 仅作 A/B 都对不齐 baseline 时兜底（SFA 源码可读但需自带构建分发，见 02 §2.3.2 和 04）。
