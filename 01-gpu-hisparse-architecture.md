...
**① FlashMLA 要什么**：它为 sparse attention 设计，不读全量 KV，而接受 `indices` 参数——"按这些行号去 KV cache tensor 里读"。每个行号是**绝对物理扁平行号**，kernel 内只做 `idx/page_block_size`、`idx%page_block_size` 拆回分页坐标(`splitkv_mla.cuh:538`)，**从不解引用 block_table**(sparse 路径的 `block_table` 是传了也被忽略的 vestigial 参数,`flash_mla_interface.py:77` 注明 "Can be None when sparse attention is used")。不关心行号怎么算来。不开 HiSparse 时行号是:indexer 给 logical 位置 → page table 在 **Python 侧**翻译成 physical 行号 → 喂给 FlashMLA。

**② HiSparse 给什么**：决定性细节——**hot buffer 不是另开的 tensor，它就是 KV cache tensor 里的一段连续区域**。所以 swap-in 写入的 `device_buffer[slot]` 本身就是 KV cache tensor 内的真实行，输出的 `top_k_device_locs` 就是这些行的 physical 行号。两边"行号"是同一件事——KV cache tensor 内的 physical row offset。FlashMLA 测试里 `indices` 也正是这么构造：
...
**所以"不需要改 FlashMLA"的本质**：HiSparse 没改 FlashMLA 的输入契约，只是把"行号清单的生产者"从 page table 换成 swap-in kernel——且因 hot buffer 复用 KV tensor 的地址空间，行号语义完全一致。这就是 GPU"只写一个 swap-in kernel 就搞定整个 HiSparse"、不动任何 attention 算子的根本原因。
